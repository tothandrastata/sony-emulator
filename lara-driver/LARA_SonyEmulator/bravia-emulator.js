...
class BraviaEmulator {
  constructor(laraEventEmitter) {
    this.laraEmitter = laraEventEmitter; // Callback to emit LARA events
  ...
  setPowerStatus(value) {
    ...
    if (changed) {
      ...
      // Emit LARA event
      if (this.laraEmitter) {
        this.laraEmitter('powerStatusChanged', newPower ? 'On' : 'Off' );
      }
    }
  ...
  setAudioMute(value) {
    ...
    this.log(`Mute ${newMute ? 'Muted' : 'Unmuted'}`);
    ...
    if (changed) {
      ...
      // Emit LARA event
      if (this.laraEmitter) {
        this.laraEmitter('muteStatusChanged',  newMute ? 'Muted' : 'Unmuted' );
      }
    }
  ...
  setInput(params) {
    ...
    const changed = this.state.input.type !== type || this.state.input.number !== number;
    ...
    if (changed) {
      ...
      // Emit LARA event
      if (this.laraEmitter) {
        this.laraEmitter('inputChanged', `${typeNames[type] || 'Unknown'} ${number}` );
      }
    }
  ...
  setSceneSetting(params) {
    ...
    if (validScenes.includes(scene)) {
      const changed = this.state.scene !== scene;
